...
With just a few simple steps, your organization’s employees can log into Stack Automation with their corporate accounts using SSO. 

SSO integration provides enhanced security and significant cost-savings as you no longer need to manage your user accounts in Stack Automation and your employees don’t need to remember another set of credentials. Quick, easy and hassle-free.

This is done by integrating your organization's identity provider with Stack Automation's identity management system. Stack Automation uses Okta for identity management and supports integration with all identity providers (IdPs).
...
2. We map your IdP's client application to Stack Automation's identity management and provide you with an SSO-enabled account.
3. A member of your organization uses this account to log in.
4. The user is redirected to Stack Automation's Corporate Account SSO (Single Sign On) login page.
5. The user enters their corporate email address and password.
6. We authenticate the user against your IdP and grant access to Stack Automation.

## Integrating your IdP with Stack Automation

1. On your IdP, create a new client application.
2. In the application, do the following:
   1. Include the users that will access Stack Automation.
   2. For each user, specify the following:
      * __space_name__: Stack Automation Space in which the user will operate.
...
5. Go to __Configuration > SSO Login__ or go to the Quali Support Center and add a new ticket, requesting SSO integration:
   ...
   Once SSO integration is set up, the application’s users will be able to use your SSO-enabled account to log in to Stack Automation, as explained in Login to Stack Automation.

> NOTE: When the user first logs in via SSO, Stack Automation sets up the user's profile, including contact details, space associations and space/account role settings. However, it is possible to update the user's profile on subsequent SSO logins as well. For details, contact Stack Automation Support.
...
* If the role name is invalid (unknown value in Stack Automation), assign the Space Member role.
...
* If the space name is invalid (unknown value in Stack Automation), ignore that space and its corresponding role.

## Mapping Stack Automation user groups to IdP groups

Each group in Stack Automation as an ‘IdP identifier’ field. For SSO users to be assigned to the group automatically, the field must contain the same identifier (usually a GUID) as the group in the IdP (the groups don’t have to have the exact same name)

In Stack Automation, this is can be set by editing a group in the ‘Groups’ page under account settings.
...
On the next login, users should be assigned automatically to groups they belong to in Azure and are mapped in Stack Automation (note it sometimes takes a short time before the claims start being sent by AAD after this change)
...
The call should contain an id_token field, the contents is a JWT token containing all the claims (user information) sent from the IdP to Stack Automation (through the browser).
...
Assuming a working Okta configuration, to get group sync working we need to have group information passed to Okta from the IdP and forwarded from it to Stack Automation with each user login.
...
Next, we need to map the groups attribute we just created (that will be filled with group data on login) to a ‘Groups’ field in the user profile that is sent to Stack Automation. Open the ‘mappings’ screen:
...
Stack Automation supports the SCIM protocol for user provisioning and deprovisioning (group provisioning is currently not supported).
...
### Relation to SSO
User provisioning (SCIM) is an optional component of SSO and can be omitted, as mentioned above, but when active, it would create a new user in Stack Automation to match a user created or associated in the IdP.
...
When that same user later logins with SSO for the first time, Stack Automation identifies that this is the same user as the one created via provisioning and performs a normal login.
When a user is removed from the IdP, they can no longer login to Stack Automation. If user provisioning was enabled at the time of deletion, then he is deleted from Stack Automation. Regardless, that user can no longer login to the account.
...
* A long token from Stack Automation with the account level permission of "Manage Users" (e.g., a token generated by an account admin)

See [**Stack Automation API Tokens**](/rest-api/torque_api_tokens) for generating a token.
...
### Okta
Login to Okta's admin console and locate the application that is used for SSO with Stack Automation under "Applications" and then "Applications" again under it.
...
At this point, user provisioning should be working. Associate a user to the app normally (under the "Assignments" tab) and observe that it gets created in the Stack Automation account. in case of an error, a red mark will appear next to the user, this links to the "Dashboard" / "Tasks" page where additional information can be found and the operation can be retried.

### Azure active directory (Entra)
Login to Azure and navigate to "Enterprise applications", then locate the application used for SSO with your Stack Automation account.
...
Associate a user to the app normally (under the "Users and groups" tab) and observe that it gets created in the Stack Automation account.
