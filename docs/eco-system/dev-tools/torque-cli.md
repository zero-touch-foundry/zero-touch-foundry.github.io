...
The Stack Automation CLI tool streamlines the YAML development process by validating changes in real-time and allowing the execution of test environments directly from the development branch, without needing to first merge the changes to production. 
...
To allow the torque-cli to authenticate with Stack Automation, you must provide several parameters:
* *Space*: The Stack Automation space to use
* *Repository*: (Optional) Represents the name of the git repository containing the blueprints and IaC files that will be used when calling Stack Automation
* *Token*: The easiest way to generate a token is via the Stack Automation UI. 
...
```bash
$ torque config list
                     Stack Automation user profiles
...
It is possible to switch the client to a different Stack Automation instance setting custom API endpoint:
...
Stack Automation CLI r

There are some basic actions Stack Automation CLI currently allows you to perform:
...
environment    Start, End, View Stack Automation environments
...
$ torque env -h
DESCRIPTION:
Start, End, View Stack Automation environments.
...
COMMANDS:
    ...
    end <ENVIRONMENT-ID>       End Stack Automation Environment
    list                       List Stack Automation Environment
    extend <ENVIRONMENT-ID>    Extend Stack Automation Environment
```