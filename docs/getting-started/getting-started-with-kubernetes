...
> NOTE: This guide will focus on Kubernetes manifest and Helm orchestration, [click here](/overview/supported-platforms.md) for more information about Stack Automation supported platform.
...
Now, that you have a new space configured, we will associate a repository to the space and generate blueprints using the automation-assets that will be discovered by Stack Automation in that repository. We recommend using your-own repository hosted in one of the supported Git Providers. For using on-prem/hosted versions of one of the supported (Version Control System) providers, see [Repository Setup](/admin-guide/source-control/source-control-github).

1. In the __repository onboarding__ wizard, choose your VCS provider, provide the repository URL and click "Connect". Stack Automation will open a new windows where you will be requested to authenticate to the VSC provider. VCS providers work differently, but usually, a token will be generated for your user and Stack Automation will use that for any communication with that VCS. 

2. Click "Discover Assets" to initiate the __Stack Automation asset discovery process__ that will go over the repository and highlight the automation assets Stack Automation discovered.
...
In this step, we will configure a Stack Automation agent that will execute, manage and monitor the Kubernetes workload we discovered and plan to deploy.
...
:::info
The command creates a new namespaces, and deploys the Stack Automation agent in it. It might take up to 2 minutes for the Agent to get up and running, so want for the Stack Automation self-service indication to make sure the agent is connected.

1. Now, it's associate our newly connected agent with our space. Click the "Associate to space".
2. The space is already selected for you, but, you still need to set the default namespace and service-account that will be used by Stack Automation to deploy workloads into the cluster. It's always possible to change the default values in the "Administration" section under the "Agents" section.
:::
