...
The Helm grain is Stack Automation's native support for Helm v3 charts. Stack Automation allows designers to use Helm specific features to easily orchestrate self-developer and community charts in a standard way and share them with others as building blocks.
...
Stack Automation will install the helm release in the namespace referred to in `target-namespace`.
...
The `source` section of a Helm grain provides Stack Automation with the information on where the Helm chart is stored and should be retrieved from. This could be either a direct source URL to a Helm chart, or it can be a reference from a Stack Automation-connected git repository.
...
Stack Automation supports referencing `values.yaml` files as inputs to the Helm grain, with the following syntax:
...
Stack Automation provides the ability to execute custom code after Stack Automation executes the Helm chart. This is useful for generating and using outputs, as explained in [helm outputs](#outputs).
