...
The Ansible grain is Stack Automation’s native support for orchestrating the execution of Ansible playbooks as part of a Stack Automation blueprint. The referenced playbook can rely on vars or inventory-hosts that are dynamically provided by Stack Automation, and then utilize them to perform configuration management, updates, a health check or any other flow that is executable from Ansible. 
...
Below is an example of an auto-generated blueprint based on a discovered playbook and its variables file. This demonstrates how Stack Automation can help you quickly scaffold a working Ansible blueprint:
...
Stack Automation is designed to streamline the installation of your module’s dependencies. It accomplishes this by automatically detecting and installing the requirements specified in a ``requirements.yaml`` or ``requirements.yml`` file. This file should be located in the root directory of your module.
...
By adhering to this structure, you can ensure that Stack Automation correctly identifies and installs all necessary requirements for your module.
...
### `source`
The source section of an Ansible grain provides Stack Automation with the information on where the Ansible playbook is stored and should be retrieved from. This could be either a direct source URL to an Ansible playbook YAML file, or it can be a reference from a Stack Automation-connected git repository. 
...
:::info
Stack Automation supports playbooks which use ansible roles.
:::
...
Stack Automation will create a JSON file containing the grain inputs under the path: /var/run/ansible/inputs/inputs.json.
...
Any section of the “inventory-file:” can contain dynamic values from dependent grains, from the blueprint’s inputs or from Stack Automation’s parameter storage. For more information, see  [Stack Automation Templating engine](/blueprint-designer-guide/blueprints/blueprints-yaml-structure#torque-templating-engine).
...
Ansible does not support outputs from playbooks natively so Stack Automation adds this support on top of the ansible capabilities.
...
For this purpose, we have developed the Stack Automation ansible module "export-torque-outputs".
...
Stack Automation allows you to run scripts before executing the Ansible playbook. This can be useful for various purposes, such as setting up the environment or performing prerequisite tasks.
...
By using pre-Ansible run scripts, you can enhance the flexibility and functionality of your Ansible playbooks within the Stack Automation ecosystem.
