...
<ProductName /> blueprints are reusable components designed to model a required environment from the infrastructure to the application. **Blueprint designers** utilize <ProductName />'s VSCode plugin or the <ProductName />'s self-service UI to build a YAML based imperative blueprints that aimed to fulfill business requirements in a self-service manner for their end-users. 
...
<ProductName /> provides different types of grains per use case:
...
## In this section
* [<ProductName />'s Blueprint YAML](/blueprint-designer-guide/blueprints/blueprints-yaml-structure)
