...
### `spec_version`
The spec_version determines the blueprint YAML type. Currently, Stack Automation supports spec_version:2 as the default and recommended version. With time, new preview releases and official feature releases will bring more and more features and users will be able to use other spec versions.
...
instructions:
  ...
  source:
    store: <The name of the repository which contains the instructions as it was onboarded to Stack Automation>
...
- Stack Automation will use only markdown files located in the “/instructions” folder under the root of the onboarded repository
- Stack Automation will support external resources embed in the markdown under the following rules:
...
- Stack Automation will not support relative .md references -Stack Automation will not allow to load/redirect to another markdown mentioned in the markdown provided as the blueprint instructions.
:::
...
The inputs section in the Stack Automation blueprint YAML also supports spaces to make inputs more user friendly. Configuring an input with a friendly name can be done in the following way:
...
To use the selected target on a grain, set it under the grain's `target` field instead of `agent`. See [target](/blueprint-designer-guide/blueprints/blueprints-yaml-structure#target) below for details, including why only one of `agent` or `target` should be provided on a grain.
...
__2. Location based on a repository (blueprints or assets) onboarded to Stack Automation__: 

The name of the repository should be provided under the 'store' field, while the IaC code folder location should be specified under the path field. In the below example, nginx helm chart resides in the 'nginx' folder within a repository onboarded to Stack Automation with the name 'web_servers'.
...
* If "tag" is provided, Stack Automation will track the repo for newer tags. In other words, if a newer tag is found, then an "update" will be detected.
* If "branch" is provided, Stack Automation will track the head of the branch. In other words, when new commits arrive, an "update" will be detected.
* If "commit" is provided, Stack Automation __will not__ track changes.
:::
...
For details on how to publish `target` as a blueprint input (including filtering the dropdown by provider type), see the [Target Input Type](/blueprint-designer-guide/blueprints/blueprints-yaml-structure#target-input-type) section above.
...
Use the `when` field on a grain to decide if that grain should run. If `when` evaluates to `false`, Stack Automation skips the grain.
...
The metadata section allows you to define additional information about the blueprint that affects how it's presented and behaves in the Stack Automation catalog. This section contains optional fields that enhance the blueprint's appearance and functionality.
...
Specifies a user-friendly display name for the blueprint that will be shown in the Stack Automation UI catalog. This allows you to provide a more descriptive name than the filename or internal identifier.
...
Icons are discovered by default under the `graphics` directory. Setting an icon in the metadata does not work for "No Repo" blueprints.
...
## Stack Automation Templating Engine
Templating engines are a great way to enrich the YAML format to allow extensibility points and text manipulations. Stack Automation utilizes a GO-Lang style engine called [Shopify Liquid](https://shopify.github.io/liquid/) to allow dynamic injection of parameters and inputs as well as provider attribute values via reference of other attributes within the same YAML. 
Example:
...
In some situations, Stack Automation will automatically retry to deploy failed grains. This behavior is usually very beneficial but it might not be suitable in all cases.
...
To use Artifactory as a binary repository in Stack Automation, you need to configure it as a workspace directory within your blueprint. Here's an example:
...
You are also able to create an Artifactory credential using the Stack Automation REST API as well. Here is an example on setting up Artifactory credentials on a space level:
