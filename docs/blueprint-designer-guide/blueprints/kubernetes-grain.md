...
The Kubernetes grain allows you to deploy native Kubernetes manifests and manifest catalogs from your repositories as part of a Stack Automation blueprint. The grain leverages Kubernetes' declarative approach to manage containerized applications and services within your Stack Automation environments.
...
Whenever a Kubernetes grain is launched, all resources created during the deployment process are automatically tagged with Stack Automation's system tags, built-in tags, and custom tags. You can disable auto-tagging for specific grains if needed.
...
Kubernetes does not natively support outputs from manifest deployments. Using Stack Automation scripts, you can extract information from deployed resources and provide it as grain outputs. Scripts are executed after the grain's installation and must export environment variables that correspond to declared outputs.
