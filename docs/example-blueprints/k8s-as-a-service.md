...
The following blueprint is build out of two grains dependent on one another. The application will be deployed only once the GKE cluster will be fully up and running including all required networking, storage and compute elements. Part of the GKE orchestration includes saving the kube config file available for the Stack Automation agent, so __the application can be installed without a Stack Automation agent in the newly created cluster__. The Stack Automation agent, will use the kube-config file to authenticate into the GKE cluster and execute the application.
