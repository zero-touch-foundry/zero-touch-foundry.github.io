...
In the following example, Terraform is utilized to orchestrate a GKE (Google Cloud managed Kubernetes cluster) cluster, while Kubernetes manifests (YAML files) are used to orchestrate an application. The capability to deploy ephemeral Kubernetes clusters on-demand, with the application already installed and configured to meet specific requirements, empowers IT and DevOps teams to enhance customer service. Moreover, this approach ensures strong oversight of deployment usage and enables optimization of both cloud resources and operational costs.

> ![GKE Deployment](/img/gke-cluster.png)

The following blueprint is build out of two grains dependent on one another. The application will be deployed only once the GKE cluster will be fully up and running including all required networking, storage and compute elements. Part of the GKE orchestration includes saving the kube config file available for the Stack Automation Management Server, so __the application can be installed without a Stack Automation Management Server in the newly created cluster__. The Stack Automation Management Server will use the kube-config file to authenticate into the GKE cluster and execute the application.
...
The deployment consumer can also download the kube-config file directly from the deployment introspection data to later connect to the cluster.
