...
- One or more target namespaces on the cluster where the Stack Automation agent will create resources.
...
- Authentication and permissions - The agent will need sufficient permissions to create the environment's cloud resources. There are a couple of ways to provide the permissions, depending on where the environment resources will be created. 
  ...
  - To create resources on your cloud using Terraform:
      ...
      - If your cluster is an __AKS__ (resources will be created on Azure): Provide the account's authentication credentials when creating the agent in Stack Automation. For details, see [Terraform Authentication on AKS](/torque-agent/service-accounts-for-azure).
...
1. In Stack Automation's **Administration** page, open the **Cloud Accounts** tab.
...
5. Paste the command in your command-line window to deploy the agent to your cluster. For example:     
    ...
    ```
5. A __Connected!__ status is displayed in Stack Automation, indicating that the agent was successfully installed and can communicate with Stack Automation. 
...
If the agent fails to connect with Stack Automation, you can try the following to identify the problem.
